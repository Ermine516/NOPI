...
import time
import itertools
from . util import format_rule, prog_size, format_prog, flatten, reduce_prog, prog_is_recursive, rule_size, rule_is_recursive, order_rule, prog_has_negation
        ...
        while True:
            ...
            with solver.solve(yield_=True) as handle:
                for m in handle:
                    ...
                    # pos_covered, model_inconsistent = self.tester.test_prog(model_prog)
                    model_inconsistent = self.tester.is_inconsistent(model_prog)
                    # print('pos_covered', len(pos_covered))
                    # print('model_inconsistent', model_inconsistent)
    ...
    def select_solution(self, new_prog):
        ...
        for prog, examples_covered in self.prog_coverage.items():
            ...
            for rule in prog:
                ...
                encoding.add('%' + format_rule(rule) + f' - {rule_id}')
            ...
            if prog_has_negation(prog):
                encoding.add(f':- {prog_rules}, #count{{1,R : rule(R)}} != {len(prog)}.')
