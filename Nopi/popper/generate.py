...
class Generator:
    ...
    def parse_model(self, model):
        ...
        pred_to_parity =  defaultdict(lambda: 0)
        neg_pred_count = 0
        pos_pred_count = 0
        neg_clauses = []
        pos_clauses = []
        ...
        for atom in model:
            args = atom.arguments
            if atom.name == 'invented':
                # duel predicate sum to 0.
                pred_to_parity[args[0].name]+= 1 if args[2].number==0 else -1
                inventeds.append(args[0].name)
            elif atom.name == 'negclause':
                neg_clauses.append(args[0].number)
            elif atom.name == 'posclause':
                pos_clauses.append(args[0].number)
        ...
        for atom in model:
            ...
            if atom.name == 'body_literal':
                ...
                arity = len(atom_args)
                par = pred_to_parity[predicate] if predicate in inventeds else 1
                body_literal = (predicate, atom_args, arity,par)
        ...
        for rule_index in rule_index_to_head:
            ...
            if rule_index in neg_clauses:
                i  = neg_clauses.index(rule_index)
                neg_clauses[i]= head_pred
            elif rule_index in pos_clauses:
                i  = pos_clauses.index(rule_index)
                pos_clauses[i]= head_pred

            for (body_pred, body_args, body_arity,parity) in rule_index_to_body[rule_index]:
                body_modes = tuple(directions[body_pred][i] for i in range(body_arity))
                if head_pred == body_pred and parity==0:
                    body.add(Literal(body_pred, body_args, body_modes,positive=True))
                elif  parity==0:
                    body.add(Literal(body_pred, body_args, body_modes,positive=False))
                else:
                    isPos = True if parity == 1 else False
                    body.add(Literal(body_pred, body_args, body_modes,positive=isPos))
        ...
        return neg_clauses,pos_clauses, frozenset(prog), rule_ordering
    ...
    def build_std_generalisation_constraint(self, prog, rule_ordering={}):
        prog = list(prog)
        rule_index = {}
        literals = []
    ...
    def build_std_specialisation_constraint(self, prog, rule_ordering={}):
        prog = list(prog)
        rule_index = {}
        literals = []
        for clause_number, rule in enumerate(prog):
        ...
        for r1, higher_rules in rule_ordering.items():
            ...
            for r2 in higher_rules:
                ...
                literals.append(lt(r1v, r2v))
        return tuple(literals)
# constraints for polarised programs
    def build_generalisation_constraint(self,pos_clauses,neg_clauses, prog,prefix,rule_ordering={},std=False):
        if len(neg_clauses)==0 or std:
            return self.build_std_generalisation_constraint(prog, rule_ordering)
        rule_index = {}
        literals = []
        prog = list(prog)
        for clause_number, rule in enumerate(prog):
            cn = vo_clause(clause_number)
            rule_index[rule] = cn
            head, body = rule
# Only positive clauses are generality constrained
            if  head.predicate in pos_clauses:
                literals.append(body_size_literal(cn, len(body)))
            if head.predicate in pos_clauses:
                literals.append(Literal(prefix[0]+'clause',(cn,)))
            else:
                literals.append(Literal(prefix[1]+'clause',(cn,)))
            literals.append(Literal('head_literal', (cn, head.predicate, head.arity, tuple(vo_variable(v) for v in head.arguments))))

            for body_literal in body:
                literals.append(Literal('body_literal', (cn, body_literal.predicate, body_literal.arity, tuple(vo_variable(v) for v in body_literal.arguments))))

            for idx, var in enumerate(head.arguments):
                literals.append(eq(vo_variable(var), idx))
# Only negative clauses are speciality constrained
        literals.append(Literal(prefix[1]+'count', (len(neg_clauses),)))
        for r1, higher_rules in rule_ordering.items():
            r1v = rule_index[r1]
            for r2 in higher_rules:
                r2v = rule_index[r2]
...
# constraints for polarised programs
    def build_specialisation_constraint(self,pos_clauses,neg_clauses, prog,prefix, rule_ordering={},std=False):
        if len(neg_clauses)==0 or std:
            return self.build_std_specialisation_constraint( prog, rule_ordering)
        prog = list(prog)
        rule_index = {}
        literals = []
        for clause_number, rule in enumerate(prog):
            ...
            rule_index[rule] = cn
            head, body = rule
    # Only negative clauses are generality constrained
            if  head.predicate in neg_clauses:
                literals.append(body_size_literal(cn, len(body)))
            if head.predicate in pos_clauses:
                literals.append(Literal(prefix[0]+'clause',(cn,)))
            else:
            ...
            for idx, var in enumerate(head.arguments):
                literals.append(eq(vo_variable(var), idx))
# Only positive clauses are speciality constrained
        literals.append(Literal(prefix[0]+'count', (len(pos_clauses),)))
...
class Grounder():
    ...
    # When grounding constraint rules, we only care about the vars and the constraints, not the actual literals
