    ...
    with generator.solver.solve(yield_ = True) as handle:
        ...
        while True:
            ...
            with settings.stats.duration('generate'):
                ...
                neg_clauses, pos_clauses, prog, rule_ordering = generator.parse_model(atoms)
            ...
            # print('settings.pi_enabled', settings.pi_enabled)
            if inconsistent:
                ...
                if len(prog) > 1 and not settings.pi_enabled:
                    for rule in prog:
                        ...
                        if tester.is_inconsistent(subprog):
                            new_cons.add(generator.build_generalisation_constraint(pos_clauses,neg_clauses,subprog,["pos","neg"],std=settings.std_con))
            else:
            ...
            if not settings.recursion_enabled:
                ...
                if combiner.solution_found:
                    for x in seen_covers_only_one_gen:
                        new_cons.add(generator.build_generalisation_constraint(pos_clauses,neg_clauses,x, ["pos","neg"],std=settings.std_con))
                    seen_covers_only_one_gen = set()
                    for x in seen_covers_only_one_spec:
                        new_cons.add(generator.build_specialisation_constraint(pos_clauses,neg_clauses,x,["pos","neg"],std=settings.std_con))
            ...
            if not inconsistent and not subsumed and len(pos_covered) > 0:
                ...
                with settings.stats.duration('combine'):
                    new_solution_found,fp = combiner.update_best_prog(prog, pos_covered)
                ...
                if new_solution_found:
                    ...
                    settings.stats.register_solution(prog, (len(pos_covered),len(settings.pos)-len(pos_covered), len(settings.neg)-fp, fp))
                    settings.stats.log_final_result()
                elif  settings.stats.solution and settings.stats.solution.conf_matrix[0] <len(pos_covered):
                     settings.stats.register_solution(prog, (len(pos_covered),len(settings.pos)-len(pos_covered), len(settings.neg)-fp, fp))
                else:
                     settings.stats.register_solution(prog, (len(pos_covered),len(settings.pos)-len(pos_covered), len(settings.neg)-fp, fp))
            ...
            if add_spec:
                new_cons.add(generator.build_specialisation_constraint(pos_clauses,neg_clauses,prog,["pos","neg"],rule_ordering,std=settings.std_con))
            if add_gen:
                new_cons.add(generator.build_generalisation_constraint(pos_clauses,neg_clauses,prog, ["pos","neg"], rule_ordering,std=settings.std_con))

            if (not settings.ban_on_neg):
