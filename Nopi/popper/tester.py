    ...
    def using(self, prog):
        ...
        try:
            for rule in prog:
                ...
                seen_nots = []
                for lit in _body:
                    if lit.predicate.split("_")[0]== "ournot" and (not lit.predicate in seen_nots):
                        lit.positve=True
                        bx= Literal(lit.predicate, lit.arguments, lit.directions)
                        self.prolog.assertz(format_rule(order_rule((lit,{bx})))[:-1])
                        current_clauses.add((lit.predicate, lit.arity))
                        seen_nots.append(lit.predicate)
