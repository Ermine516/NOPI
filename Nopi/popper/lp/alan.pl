...
%% ********** NEGATIVE INVENTION RULES **********
polarity(0..1).

polarityEq(Ph,1,Pb):- head_literal(C,Ph,_,_),
                      body_literal(C,@neg_bk_create(Pb),_,_),
                      invented(Pb,_).
polarityEq(Ph,0,Pb):- head_literal(C,Ph,_,_),
                      body_literal(C,Pb,_,_),
                      invented(Pb,_).

polarityEq(P,0,P):- head_literal(C,P,_,_),
                    not body_literal(_,P,_,_),
                    not body_literal(_,@neg_bk_create(P),_,_).

polarityEq(Ph,X^Y,Pb2):-
                     head_pred(Ph,_),
                     polarityEq(Ph,X,Pb),
                     polarityEq(Pb,Y,Pb2).

negclause(C):- head_pred(Ph,_),
               polarityEq(Ph,1,Pb),
               head_literal(C,Pb,_,_).

posclause(C):- head_pred(Ph,_),
               polarityEq(Ph,0,Pb),
               head_literal(C,Pb,_,_).

%% Count of negative clauses
negcount(X):- #count{C: negclause(C)}=X.
%% Count of positve clauses
poscount(X):- #count{C: posclause(C)}=X.
% Is Polar if every
:-clause(C),posclause(C),negclause(C).
...
body_aux(P,A):-
    invented(P,A),
    not enable_nepi. %takes forever without this @TODO
body_aux(@neg_bk_create(P),A):-
    invented(P,A),
    enable_nepi.
...
calls_invented(Rule):-
    invented(P,A),
    body_literal(Rule,@neg_bk_create(P),A,_).
:-
    pi,
    not recursive,
...
def neg_bk_check(x,y):
    try:
      ret = Number(x.arguments.index(Function(y.name)))
      return ret
    except ValueError as ve:
      return Number(-1)
def neg_bk_create(sym):
      return Function("ournot_"+sym.name)
def neg_bk_dir(a):
      ret = Function("",[Function("in") for x in range(0,a.number)])
      return ret
#end.

body_pred(@neg_bk_create(X),A):- 0<=@neg_bk_check(Z,X), neg_bk(Z),body_pred(X,A).
type(@neg_bk_create(X),T):- 0<=@neg_bk_check(Z,X), neg_bk(Z) ,type(X,T).
direction(@neg_bk_create(X),@neg_bk_dir(A)):- 0<=@neg_bk_check(Z,X), neg_bk(Z) ,direction(X,_),body_pred(X,A).

% A literal cannot show up with it's negation
:- body_literal(C,X,A,Vars), body_literal(C,@neg_bk_create(X),A,Vars).
...
%%% TODO Breaks NOPI. Removed for now
% :-
%     Rule > 0,
...
direction_(P,Pos,in):-
    Pos=0..A-1,
    invented(P,A).
direction_(@neg_bk_create(P),Pos,in):-
    Pos=0..A-1,
    invented(P,A),
    enable_nepi.
% direction_(P,Pos,in):-
...
appears_before(P,A):-
    ...
    lower(Q,P),
    head_literal(C,Q,_,_),
    body_literal(C,@neg_bk_create(P),_,_).
...
:-
    head_literal(C,P,_,_),
    body_literal(C,@neg_bk_create(Q),_,_),
    lower(Q,P).
...
var_type(C1,Var1,Type):-
    ...
    C1 > C2,
    head_literal(C1,P,A,Vars1),
    body_literal(C2,@neg_bk_create(P),A,Vars2),
    var_pos(Var1,Vars1,Pos),
    var_pos(Var2,Vars2,Pos),
    var_type(C2,Var2,Type).
...
%% TODO: REDUCE GROUNDING USING ORDERING OVER INVENTED SYMBOLS
%% a literal appears in a positive rule
positive_literal(Rule,Q,Vars):-
    posclause(Rule),
    body_literal(Rule,Q,_,Vars).

%% a literal appears in a negative rule
negative_literal(Rule,Q,Vars):-
    negclause(Rule),
    invented(P,_),
    head_literal(Rule,P,_,Vars),
    body_literal(Rule,Q,_,Vars).

%% a literal cannot appear both positively and negatively
:-
    positive_literal(Rule1,P,Vars),
    negative_literal(Rule2,P,Vars),
    Rule1 != Rule2.
