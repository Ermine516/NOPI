...
with solver.solve(yield_ = True) as handle:
    ...
    if previous != None:
        ...
        realoutput.append(":- begin_tests(propersubgraph).\n")
        for x in range(0,int(graphs)):
        		realoutput.append("test("+"posa):- propersubgraph("+str(x)+","+str(x+int(graphs))+").\n")
        		realoutput.append("test("+"nega):- \+ propersubgraph("+str(x+2*int(graphs))+","+str(x+3*int(graphs))+").\n")
        realoutput.append(":- end_tests(propersubgraph).\n")
