...
with solver.solve(yield_ = True) as handle:
    ...
    if previous != None:
        ...
        realoutput.append(":- begin_tests(tests).\n")
        for g,s in outputexs.items():
                if  int(graphs) <= int(g) and int(g) <2*int(graphs) :
                	realoutput.append("test("+"pos"+g+"):- dominating("+str(int(g)-int(graphs))+",["+",".join(s)+"]).\n")
                else:
                    realoutput.append("test("+"neg"+g+"):- \+ dominating("+str(int(g)-int(graphs))+",["+",".join(s)+"]).\n")
        	realoutput.write(":- end_tests(tests).\n")
        	realoutput.write(":- initialization go.\n go :- run_tests(tests), halt.")
